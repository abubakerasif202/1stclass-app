...
class LocationTrackingService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        // The app reconciles persisted shift/session state when it is next opened. Avoid Android
        // independently resurrecting tracking after logout or an explicit stop.
        return START_NOT_STICKY
    }
    ...
    private suspend fun beginIfAllowed() {
            ...
            override fun onLocationResult(result: LocationResult) {
                result.locations.forEach { location ->
                    ...
                    val point = LocationPoint(
                        id = UUID.randomUUID().toString(), driverId = shift.driverId, shiftId = shift.id,
                        latitude = location.latitude, longitude = location.longitude,
                        accuracyMeters = location.accuracy,
                        speedMetersPerSecond = location.speed.takeIf { location.hasSpeed() },
                        bearingDegrees = location.bearing.takeIf { location.hasBearing() },
                        altitudeMeters = location.altitude.takeIf { location.hasAltitude() },
                        recordedAt = location.time, createdAt = System.currentTimeMillis()
                    )
                    scope.launch {
        ...
        val request = LocationRequest.Builder(Priority.PRIORITY_BALANCED_POWER_ACCURACY, 20_000L)
    ...
    private fun createChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            getSystemService(NotificationManager::class.java).createNotificationChannel(
                NotificationChannel(CHANNEL_ID, "Shift Tracking", NotificationManager.IMPORTANCE_LOW).apply {
                    description = "Active-shift location tracking"
                }
            )
        }
    }
    ...
    private fun notification() = NotificationCompat.Builder(this, CHANNEL_ID)
        ...
        .setContentText("Shift location tracking active")
        .setOngoing(true)
